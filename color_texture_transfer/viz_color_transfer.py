...
def transfer_color_lut_with_checkpoints(content, prob, style_img, bins=33, n_iter=800, lr=0.001, tv_weight=10):
    ...
    for it in range(n_iter):
        ...
        if (it + 1) % 100 == 0:
            with torch.no_grad():
                ...
                checkpoints.append(out * 2.0 - 1.0)
...
def main():
    ...
    content_path = coco_paths[seed]
    style_path = wiki_paths[20000 + seed]
    print(f"Fixed content: {content_path.name}")
    print(f"Fixed style  : {style_path.name}")
    ...
    # 颜色迁移，每100步返回一个checkpoint
    print("Running color LUT optimization (800 steps)...")
    color_checkpoints = transfer_color_lut_with_checkpoints(
        content_img, P, style_img,
        bins=33, n_iter=800, lr=0.001, tv_weight=10
    )

    # 对每个checkpoint做纹理迁移
    print("Running texture transfer for each checkpoint...")
    results = []
    for idx, cp in enumerate(color_checkpoints):
        tex_result, _, _ = transfer_texture_rgb(
            cp, style_img, texture_model,
            n_clusters=8, gf_radius=2, gf_eps=0.01,
            use_feather=True, feather_sigma=1.5
        )
        results.append(tex_result[0])
        print(f"  Step {(idx+1)*100} done.")
    ...
    imgs = [
        to_pil(content_img[0]), to_pil(results[0]), to_pil(results[1]), to_pil(results[2]), to_pil(results[3]),
        to_pil(style_img[0]),   to_pil(results[4]), to_pil(results[5]), to_pil(results[6]), to_pil(results[7]),
    ]
    labels = [
        "内容图像", "100步", "200步", "300步", "400步",
        "风格图像", "500步", "600步", "700步", "800步",
    ]
    ...
    W, H = imgs[0].size
    canvas = Image.new("RGB", (W * 5, H * 2), (30, 30, 30))
    ...
    print(f"\nSaved 2×5 grid to {save_path}")

    # 打印初始和最终KL
    kl_before = compute_kl(content_01, P, bins=33)
    kl_after = compute_kl((color_checkpoints[-1] + 1.0) / 2.0, P, bins=33)
