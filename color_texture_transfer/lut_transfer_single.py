...
def save_split_vis(labels, path, size=(256, 256)):
    """保存纹理分割可视化（彩色聚类图）"""
    seg = label_color(labels)
    pil = Image.fromarray(seg)
    if pil.size != size:
        pil = pil.resize(size, Image.NEAREST)
    pil.save(path)
    print(f"Saved split visualization to {path}")


def save_full_vis(I, S, I_color, I_texture, labels_c, labels_s, path):
    """保存完整对比图：内容图 | 内容分割 | 风格图 | 风格分割 | 仅颜色 | 颜色+纹理"""
    def to_pil(t):
        t = ((t.detach() + 1.0) / 2.0).clamp(0, 1)
        arr = (t.permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
        return Image.fromarray(arr)

    seg_c = label_color(labels_c)
    seg_s = label_color(labels_s)
    seg_c_pil = Image.fromarray(seg_c).resize(to_pil(I).size, Image.NEAREST)
    seg_s_pil = Image.fromarray(seg_s).resize(to_pil(S).size, Image.NEAREST)

    imgs = [
        to_pil(I), seg_c_pil,
        to_pil(S), seg_s_pil,
        to_pil(I_color), to_pil(I_texture),
    ]

    W, H = imgs[0].size
    canvas = Image.new("RGB", (W * 6, H), (30, 30, 30))
    for idx, img in enumerate(imgs):
        canvas.paste(img, (idx * W, 0))

    canvas.save(path)
    print(f"Saved full visualization to {path}")
...
def main():
    ...
    style_path   = demo_dir / "s.png"
    content_path = demo_dir / "c.png"
    ckpt_color   = Path("checkpoints/latest.pth")
    ckpt_texture = Path("checkpoints/encoder.pth")

    if not style_path.exists():
        raise FileNotFoundError(f"风格图不存在: {style_path}")
    if not content_path.exists():
        raise FileNotFoundError(f"内容图不存在: {content_path}")
    ...
    texture_model.load_state_dict(torch.load(ckpt_texture, map_location=device)["model_state_dict"])
    texture_model.eval()
    ...
    print(f"KL before: {kl_before:.4f}")

    I_color = transfer_color_lut(content_img, P, style_img,
                                 bins=33, n_iter=800, lr=0.001, tv_weight=10)

    with torch.no_grad():
        kl_after = compute_kl((I_color + 1.0) / 2.0, P, bins=33)
    print(f"KL after : {kl_after:.4f}")

    # ---- Step B: 纹理迁移 ----
    print("\n>>> Step 2: Texture transfer")
    I_texture, labels_c, labels_s = transfer_texture_rgb(
        I_color, style_img, texture_model,
        n_clusters=8, gf_radius=2, gf_eps=0.01,
        use_feather=True, feather_sigma=1.5
    )
    print("Texture transfer done.")

    # ---- 保存分割可视化 ----
    save_split_vis(labels_c, demo_dir / "c_split.png", size=(256, 256))
    save_split_vis(labels_s, demo_dir / "s_split.png", size=(256, 256))

    # ---- 保存完整对比图 ----
    save_full_vis(content_img[0], style_img[0],
                  I_color[0], I_texture[0],
                  labels_c, labels_s, demo_dir / "result.png")

    print(f"\nAll outputs saved to: {demo_dir.resolve()}")
