...
def transfer_color_lut(content, prob, style_img, lambda_mix=0.5, bins=33, n_iter=N_ITER, lr=0.001, tv_weight=10):
...
def main():
    ...
    # 同时计算 VGG 风格损失 与 KL 散度
    n_lams = len(LAMBDAS)
    vgg_losses = np.zeros((NUM_IMAGES, n_lams))
    kl_losses = np.zeros((NUM_IMAGES, n_lams))
    direct_vgg = np.zeros(NUM_IMAGES)
    direct_kl = np.zeros(NUM_IMAGES)
    ...
    for i in range(NUM_IMAGES):
        ...
        with torch.no_grad():
            ...
            P = prob[0]
            direct_vgg[i] = vgg_style(content, style).item()
            direct_kl[i] = compute_kl(content_01, P, bins=33)

        for j, lam in enumerate(LAMBDAS):
            I_color = transfer_color_lut(
                content, P, style,
                lambda_mix=lam, bins=33, n_iter=N_ITER, lr=0.001, tv_weight=10
            )
            ...
            with torch.no_grad():
                vgg_losses[i, j] = vgg_style(I_texture, style).item()
                kl_losses[i, j] = compute_kl((I_texture + 1.0) / 2.0, P, bins=33)

        print(f"Image {i+1}/{NUM_IMAGES} done. "
              f"Avg VGG: {vgg_losses[i].mean():.4f}, Avg KL: {kl_losses[i].mean():.4f}, "
              f"Direct VGG: {direct_vgg[i]:.4f}, Direct KL: {direct_kl[i]:.4f}")

    # 保存 CSV：每个 lambda 同时给出 VGG 与 KL，最后两列是 direct 基准
    csv_path = "lambda_style_loss.csv"
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        header = ['image_id']
        for l in LAMBDAS:
            header += [f'vgg_{l:.1f}', f'kl_{l:.1f}']
        header += ['direct_vgg', 'direct_kl']
        writer.writerow(header)

        for i in range(NUM_IMAGES):
            row = [i + 1]
            for j in range(n_lams):
                row += [vgg_losses[i, j], kl_losses[i, j]]
            row += [direct_vgg[i], direct_kl[i]]
            writer.writerow(row)
