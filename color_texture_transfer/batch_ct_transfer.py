...
def save_vis_with_labels(I, S, I_color, I_texture, path, font):
    def to_pil(t):
    ...
    for i in range(len(imgs)):
        arr = np.array(imgs[i])
        arr = draw_label(arr, labels_text[i], font)
        imgs[i] = Image.fromarray(arr)

    W, H = imgs[0].size
    canvas = Image.new("RGB", (W * 4, H), (30, 30, 30))
...
def main():
    ...
    for i in range(100):
        ...
        save_vis_with_labels(
            content_img[0], style_img[0],
            I_color[0], I_texture[0],
            str(save_path), font
        )

        print(f"\n[Sample {i+1}] KL before: {kl_before:.4f} | KL after: {kl_after:.4f} | Saved: {rand_name}")

    print(f"\nDone. 20 images saved to {out_dir.resolve()}")
