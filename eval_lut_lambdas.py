...
# Matplotlib 中文设置（Windows SimHei）
...
def transfer_color_lut(content, prob, style_img, lambda_mix=0.5, bins=33, n_iter=N_ITER, lr=0.001, tv_weight=10, verbose=False):
    ...
    with torch.no_grad():
        ...
        P_target = lambda_mix * prob + (1 - lambda_mix) * P_style
    ...
    for it in range(n_iter):
        ...
        if verbose and (it % 50 == 0 or it == n_iter - 1):
            print(f"  [Color LUT] Iter {it+1}: KL = {kl.item():.4f}")
...
# ========================== 8. 可视化工具 ==========================
def tensor_to_pil(t, size=(256, 256)):
    t = ((t.detach().clamp(-1, 1) + 1.0) / 2.0)
    if t.dim() == 4:
        t = t[0]
...
def make_comparison_figure(content_img, style_img, transferred_list, lambdas, save_path):
    W, H = 256, 256

    left_col = Image.new("RGB", (W, H * 2))
    left_col.paste(tensor_to_pil(content_img, (W, H)), (0, 0))
    left_col.paste(tensor_to_pil(style_img, (W, H)), (0, H))

    right_grid = Image.new("RGB", (W * 5, H * 2))
    draw = ImageDraw.Draw(right_grid)

    # Windows 中文字体：优先 SimHei（黑体），其次微软雅黑，最后默认
    font = None
    for font_name in ["simhei.ttf", "msyh.ttc", "SimHei", "Microsoft YaHei"]:
        try:
    ...
    for idx, (img_tensor, lam) in enumerate(zip(transferred_list, lambdas)):
        row = idx // 5
        col = idx % 5
        x = col * W
        y = row * H
        pil = tensor_to_pil(img_tensor, (W, H))
        right_grid.paste(pil, (x, y))

        text = f"λ={lam:.1f}"
        bbox = draw.textbbox((0, 0), text, font=font)
        tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
        draw.rectangle([x + 5, y + 5, x + 5 + tw + 4, y + 5 + th + 4], fill=(0, 0, 0))
        draw.text((x + 7, y + 7), text, fill=(255, 255, 255), font=font)

    total = Image.new("RGB", (W + W * 5, H * 2))
    total.paste(left_col, (0, 0))
    total.paste(right_grid, (W, 0))
...
def main():
    ...
    random.seed(seed)
    np.random.seed(seed)
    print(f"Device: {device}, Seed: {seed}, Images: {NUM_IMAGES}, LUT iter: {N_ITER}")
    ...
    if len(coco_paths) < NUM_IMAGES or len(wiki_paths) < NUM_IMAGES:
        raise RuntimeError(f"Not enough images: coco={len(coco_paths)}, wiki={len(wiki_paths)}, need={NUM_IMAGES}")

    random.shuffle(coco_paths)
    random.shuffle(wiki_paths)
    coco_paths = coco_paths[:NUM_IMAGES]
    wiki_paths = wiki_paths[:NUM_IMAGES]

    contents, styles = [], []
    for c_path, s_path in zip(coco_paths, wiki_paths):
    ...
    # lambda 扫描 + 原始差距
    n_lams = len(LAMBDAS)
    style_losses = np.zeros((NUM_IMAGES, n_lams))
    direct_losses = np.zeros(NUM_IMAGES)  # 内容图与风格图直接算 Gram，不迁移
    ...
    for i in range(NUM_IMAGES):
        ...
        # 计算原始差距（未迁移）
        with torch.no_grad():
            direct_losses[i] = vgg_style(content, style).item()

        with torch.no_grad():
            prob, _ = color_model(style)
            P = prob[0]

        transferred_list = []
        for j, lam in enumerate(LAMBDAS):
            I_color = transfer_color_lut(
                content, P, style,
                lambda_mix=lam, bins=33, n_iter=N_ITER, lr=0.001, tv_weight=10, verbose=False
            )
            I_texture, _, _ = transfer_texture_rgb(
                I_color, style, texture_model,
                n_clusters=8, gf_radius=2, gf_eps=0.01,
                use_feather=True, feather_sigma=1.5
            )
            with torch.no_grad():
                s_loss = vgg_style(I_texture, style)
            style_losses[i, j] = s_loss.item()
            transferred_list.append(I_texture)

        out_path = out_dir / f"{i+1:03d}.png"
        make_comparison_figure(content, style, transferred_list, LAMBDAS, out_path)
        print(f"Image {i+1}/{NUM_IMAGES} done. Avg style loss: {style_losses[i].mean():.4f}, Direct: {direct_losses[i]:.4f}")

    # 统计 best / worst（以迁移后的平均风格损失为准）
    avg_style_per_img = style_losses.mean(axis=1)
    best_idx = int(np.argmin(avg_style_per_img))
    worst_idx = int(np.argmax(avg_style_per_img))
    print(f"\nBest case  -> example/{best_idx+1:03d}.png (avg style loss {avg_style_per_img[best_idx]:.4f})")
    print(f"Worst case -> example/{worst_idx+1:03d}.png (avg style loss {avg_style_per_img[worst_idx]:.4f})")

    # 柱状图：10 个 λ + 1 个原始差距
    fig, ax = plt.subplots(figsize=(12, 6))
    x = np.arange(n_lams + 1)
    means = np.append(style_losses.mean(axis=0), direct_losses.mean())
    stds = np.append(style_losses.std(axis=0), direct_losses.std())

    colors = ['coral'] * n_lams + ['gray']
    labels = [f"λ={l:.1f}" for l in LAMBDAS] + ["原始差距"]

    bars = ax.bar(x, means, yerr=stds, capsize=4, color=colors, edgecolor='black', alpha=0.85)

    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_xlabel("λ（模型预测权重）")
    ax.set_ylabel("VGG 风格损失（Gram MSE）")
    ax.set_title(f"风格损失 vs. λ（{NUM_IMAGES} 张图，均值 ± 标准差）")
    ax.grid(axis='y', linestyle='--', alpha=0.4)

    # 添加图例说明：珊瑚色 = 迁移后，灰色 = 未迁移原始差距
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor='coral', edgecolor='black', label='颜色+纹理迁移'),
        Patch(facecolor='gray', edgecolor='black', label='内容图与风格图直接计算（未迁移）')
    ]
    ax.legend(handles=legend_elements, loc='upper right')

    plt.tight_layout()
    bar_path = out_dir / "lambda_style_loss.png"
