...
def main():
    ...
    if not coco_imgs or not wiki_classes:
    ...
    # 加载纹理迁移 Encoder（CT 需要；也是 ED 的 encoder）
    ...
    # CSV
    out_path = Path("unified_results_128.csv")
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["style_name", "direct_loss", "flow_matching", "color_only", "color_texture", "single_transfer"])
    ...
    for class_name, style_files in sorted(wiki_classes.items()):
        ...
        print(f"\n{'='*60}")
        print(f"Style: {class_name} | {NUM_PER_STYLE} samples")
        print(f"{'='*60}")

        # 预加载所有图像 tensor (128×128, [-1,1])
        content_tensors = []
        style_tensors = []
        for i in range(NUM_PER_STYLE):
            cp = content_batch[i]
            sp = style_files[i % len(style_files)]
            ct = TRANSFORM(Image.open(cp).convert("RGB")).unsqueeze(0).to(DEVICE)
            st = TRANSFORM(Image.open(sp).convert("RGB")).unsqueeze(0).to(DEVICE)
        ...
        for i in range(NUM_PER_STYLE):
            with torch.no_grad():
                direct_losses.append(vgg_loss(content_tensors[i], style_tensors[i]).item())

        # 2) 流匹配特征预提取 + batch 推理
        fm_losses = [0.0] * NUM_PER_STYLE
        ...
        for b_start in range(0, NUM_PER_STYLE, FM_BATCH_SIZE):
            b_end = min(b_start + FM_BATCH_SIZE, NUM_PER_STYLE)
            ...
            for j in range(actual_bs):
                idx = b_start + j
                with torch.no_grad():
                    fm_losses[idx] = vgg_loss(gen[j:j+1], style_tensors[idx]).item()

        # 3) 颜色迁移 (仅颜色，无纹理)
        color_only_losses = [0.0] * NUM_PER_STYLE
        ct_losses = [0.0] * NUM_PER_STYLE
        with torch.no_grad():
            style_cat = torch.cat(style_tensors, dim=0)
            probs, _ = color_model(style_cat)
        for i in range(NUM_PER_STYLE):
            I_color = transfer_color_lut(content_tensors[i], probs[i], style_tensors[i],
                                         bins=33, n_iter=800, lr=0.001, tv_weight=10)
            with torch.no_grad():
                color_only_losses[i] = vgg_loss(I_color, style_tensors[i]).item()

            I_ct, _, _ = transfer_texture_rgb(
                I_color, style_tensors[i], encoder,
                n_clusters=8, gf_radius=2, gf_eps=0.01,
                use_feather=True, feather_sigma=1.5
            )
            with torch.no_grad():
                ct_losses[i] = vgg_loss(I_ct, style_tensors[i]).item()

        # 5) Encoder-Decoder 单次迁移 (逐张，Q 替换限制 B=1)
        ed_losses = [0.0] * NUM_PER_STYLE
        for i in range(NUM_PER_STYLE):
            ...
            with torch.no_grad():
                ...
                ed_losses[i] = vgg_loss(rec, st).item()
        ...
        with open(out_path, "a", newline="", encoding="utf-8") as f:
            ...
            for i in range(NUM_PER_STYLE):
                writer.writerow([
                    class_name,
                    fmt4(direct_losses[i]),
                    fmt4(fm_losses[i]),
                    fmt4(color_only_losses[i]),
                    fmt4(ct_losses[i]),
                    fmt4(ed_losses[i]),
                ])

        # 打印本类平均
        print(f"  Direct:     {np.mean(direct_losses):.4g}")
        print(f"  Flow:       {np.mean(fm_losses):.4g}")
        print(f"  ColorOnly:  {np.mean(color_only_losses):.4g}")
        print(f"  CT:         {np.mean(ct_losses):.4g}")
        print(f"  Single:     {np.mean(ed_losses):.4g}")
